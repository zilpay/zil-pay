...
async function retrieveService(device: BluetoothDevice) {
  if (!device.gatt)
    throw new TransportError("Bluetooth GATT not found", "GATTNotFound");
  const [service] = await device.gatt.getPrimaryServices();
  if (!service)
    throw new TransportError("Bluetooth service not found", "ServiceNotFound");
  const infos = getInfosForServiceUuid(service.uuid);
  if (!infos)
    throw new TransportError(
      "Bluetooth service infos not found",
      "ServiceInfosNotFound",
    );
...
async function sendAPDU(
  ...
  const chunks = createChunkedBuffers(
    apdu,
    (i) => mtuSize - (i === 0 ? 5 : 3),
  ).map((buffer, i) => {
...
async function open(
  ...
  const transport = new TransportWebBLE(
    device,
    writeC,
    notifyObservable,
    deviceModel,
...
export default class TransportWebBLE extends Transport {
  ...
  static listen = (
    observer: Observer<DescriptorEvent<Device>>,
  ): Subscription => {
