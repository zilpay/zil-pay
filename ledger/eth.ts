...
export class EthLedgerInterface {
  ...
  async getAddress(
    ...
    const publicKey = uint8ArrayToHex(
      response.subarray(1, 1 + publicKeyLength),
    );
    ...
    const address =
      HEX_PREFIX +
      uint8ArrayToUtf8(
        response.subarray(addressOffset + 1, addressOffset + 1 + addressLength),
      );
