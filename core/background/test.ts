...
export async function testFromOld(core: ZIlPayBackground) {
  ...
  core.popup.initPopup(({ resolve }) => {
    ...
    assert(resolve['guard']['isReady'], 'Guard should be ready.');
    assert(resolve['guard']['isEnable'], 'Guard should be enabled.');

    assert(resolve['netwrok'].selected === 'mainnet', 'Netwrok should be mainnet');
