...
describe('Gas Parser Utilities', () => {
  ...
  const mockEvmSender = new Address(new Uint8Array(20), AddressType.EthCheckSum);
  ...
  describe('buildFeeHistoryRequest', () => {
    it('should create a valid eth_feeHistory request payload', () => {
      ...
      expect(request.params).toEqual([`0x${blockCount.toString(16)}`, 'latest', percentiles]);
  ...
  describe('buildEvmEstimateGasRequest', () => {
    ...
    it('should throw NetworkError if the transaction is for Scilla', () => {
      const tx = new TransactionRequest({
        chainHash: 0,
      }, mockScillaTx as any, undefined);
      expect(() => buildEvmEstimateGasRequest(tx)).toThrow("Zilliqa network doesn't support gas estimation or EVM transaction is missing");
    });
  });

  describe('processParseFeeHistoryRequest', () => {
    const mockFeeHistory: FeeHistoryResult = {
      baseFeePerGas: ["0x21149ce3", "0x20fde27c", "0x1fa69dae", "0x1e3966e7", "0x1fa68dd6"],
      reward: [
    ...
    it('should correctly parse a valid fee history response', () => {
      const result: GasFeeHistory = processParseFeeHistoryRequest(mockFeeHistory);
    ...
    it('should throw an error if baseFeePerGas is missing', () => {
      ...
      expect(() => processParseFeeHistoryRequest(invalidHistory)).toThrow('baseFeePerGas not found or empty in fee history');
    });
  ...
  describe('buildBatchGasRequest', () => {
    it('should build a batch request for a Zilliqa (Scilla) transaction', async () => {
      const tx = new TransactionRequest({ chainHash: 1 }, mockScillaTx as any, undefined);
      const requests = await buildBatchGasRequest(tx, 0, [], [], mockZilSender);

      expect(requests.length).toBe(2);
      expect(requests[0].method).toBe(ZilMethods.GetBalance);
      expect(requests[0].params).toEqual(['0000000000000000000000000000000000000000']); 
    ...
    it('should build a basic batch request for an EVM transaction', async () => {
      const tx = new TransactionRequest({ chainHash: 2 }, undefined, mockEvmTx as any);
      const requests = await buildBatchGasRequest(tx, 10, [], [], mockEvmSender);

      expect(requests.length).toBe(3);
      expect(requests[0].method).toBe(EvmMethods.GetTransactionCount);
      expect(requests[0].params).toEqual(['0x0000000000000000000000000000000000000000', 'latest']);
    ...
    it('should include EIP1559 requests when the feature is enabled', async () => {
      const tx = new TransactionRequest({ chainHash: 2 }, undefined, mockEvmTx as any);
      const requests = await buildBatchGasRequest(tx, 10, [25, 75], [EIP1559], mockEvmSender);
    ...
    it('should include EIP4844 request when the feature is enabled', async () => {
      const tx = new TransactionRequest({ chainHash: 2 }, undefined, mockEvmTx as any);
      const requests = await buildBatchGasRequest(tx, 10, [], [EIP4844], mockEvmSender);
