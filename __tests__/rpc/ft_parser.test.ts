...
const mockZilBase16Address = await Address.fromStr('0x000000000000000000000000000000000000dead');

const mockEthAddress = new Address(new Uint8Array(20), AddressType.EthCheckSum);
const mockZilAddress = new Address(new Uint8Array(20), AddressType.Bech32);
...
describe('ft_parser', () => {
  ...
  describe('ERC20Helper', () => {
    ...
    it('should correctly encode "symbol" function call', () => {
      ...
      // 0x95d89b41 - селектор для symbol()
    ...
    it('should correctly encode "decimals" function call', () => {
      ...
      // 0x313ce567 - селектор для decimals()
    ...
    it('should correctly encode "balanceOf" function call', () => {
      const encoded = helper.encodeFunctionCall('balanceOf', [toAddress]);
       // 0x70a08231 - селектор для balanceOf(address)
    ...
    it('should correctly encode "transfer" function call', () => {
      ...
      // 0xa9059cbb - селектор для transfer(address,uint256)
    ...
    it('should generate transfer input data', () => {
        ...
        const expectedData = '0xa9059cbb00000000000000000000000001010101010101010101010101010101010101010000000000000000000000000000000000000000000000000de0b6b3a7640000';
  ...
  describe('buildTokenRequests', () => {
    it('should build requests for an ETH (ERC20) token', async () => {
      const accounts = [mockEthAddress];
      const requests = await buildTokenRequests(mockEthAddress, accounts, false);

      // 3 metadata requests (name, symbol, decimals) + 1 balance request
      expect(requests).toHaveLength(4);

      const metadataReqs = requests.filter(r => r.requestType.type === 'Metadata');
    ...
    it('should build requests for a native ETH balance', async () => {
        const accounts = [mockEthAddress];
        const requests = await buildTokenRequests(mockEthAddress, accounts, true);
    ...
    it('should build requests for a ZIL (ZRC2) token', async () => {
      const accounts = [mockZilAddress];
      const requests = await buildTokenRequests(mockZilAddress, accounts, false);
    ...
    it('should build requests for a native ZIL balance', async () => {
        const accounts = [mockZilAddress];
        const requests = await buildTokenRequests(mockZilAddress, accounts, true);
  ...
  describe('Response Processing', () => {
    describe('ETH', () => {
        it('should process ETH metadata response for name', () => {
            ...
            const name = processEthMetadataResponse(mockResponse, MetadataField.Name);
          ...
          it('should process ETH metadata response for decimals', () => {
            ...
            const decimals = processEthMetadataResponse(mockResponse, MetadataField.Decimals);
          ...
          it('should throw error for invalid ETH response', () => {
            ...
            expect(() => processEthMetadataResponse(mockResponse, MetadataField.Name)).toThrow('RPC Error (code: -32000): Invalid request');
          });
    });
    ...
    describe('ZIL', () => {
        it('should process ZIL metadata response', () => {
            ...
            const { name, symbol, decimals } = processZilMetadataResponse(mockResponse);
          ...
          it('should throw error for missing field in ZIL metadata', () => {
            const mockInitData: GetTokenInitItem[] = [
              ...
              expect(() => processZilMetadataResponse(mockResponse)).toThrow('Invalid contract init: missing symbol');
          });
          ...
          it('should process native ZIL balance response', async () => {
            ...
            const balance = await processZilBalanceResponse(mockResponse, mockZilAddress, true);
            expect(balance).toBe(12345n);
          });

          it('should process ZRC2 token balance response', async () => {
            const balances = { [mockZilBase16Address.toBase16()]: '54321' };
            ...
            const balance = await processZilBalanceResponse(mockResponse, mockZilAddress, false);
            expect(balance).toBe(54321n);
        });

        it('should return 0 for ZRC2 balance if account not in response', async () => {
            // Create a different address for the test
            const otherAddress = new Address(new Uint8Array(20).fill(1), AddressType.Bech32);
            vi.spyOn(otherAddress, 'toZilChecksum').mockResolvedValue('anotheraddress');
            ...
            const mockResponse: RpcResult<ZilSmartContractSubStateResponse> = {
                ...
                result: {
                    balances: {
                        'someotheraddress': '100'
                    }
                },
            };
    
            const balance = await processZilBalanceResponse(mockResponse, otherAddress, false);
