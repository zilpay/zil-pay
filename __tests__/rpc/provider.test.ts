...
import { RpcProvider, RpcError } from '../../background/rpc/provider';
...
const createZilliqaConfig = (): ChainConfig =>
  new ChainConfig({
    ...
    chain: 'ZIL',
    logo: '',
    rpc: ['https://ssn.zilpay.io/api', 'https://ssn-api-mainnet.viewblock.io', 'https://zilliqa.avely.fi/api', 'https://ssn.zillet.io', 'https://api.zilliqa.com'],
    features: [],
    chainIds: [1, 0],
    slip44: 313,
    explorers: [],
...
describe('RpcProvider', () => {
  ...
  it('test_get_balance_scilla', async () => {
    ...
    const payload = zil.buildPayload(ZilMethods.GetBalance, [address.replace("0x", "").toLowerCase()]);
  ...
  it('test_batch_request', async () => {
    ...
    const bsc = new RpcProvider(netConf);
    const payloads = [
        bsc.buildPayload(EvmMethods.GetBalance, [
            ZERO_ADDR_HEX,
            'latest',
        ]),
    ...
    const res = await bsc.req<[EthResultRes<string>, EthResultRes<string>]>(payloads);
  ...
  it('should rotate nodes on failure and succeed on retry', async () => {
    ...
    const payload = provider.buildPayload(EvmMethods.GetBalance, [ZERO_ADDR_HEX, 'latest']);
